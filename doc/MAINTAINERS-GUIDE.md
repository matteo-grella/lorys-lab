...
| `mirror` | 68×14 | ✓ | ✓ (sandbox) | ✓ | – | rotatable 360°, DOUBLE-SIDED glass: reflects laser/lens beams specularly off whichever face is struck (the normal flips to the struck side — a one-sided mirror left half the rotation dial dead, hostile to kids aiming it). The beam becomes a polyline (`lab.laz/lens.beamPath`, world coords, drawn in render's beam pass ABOVE parts) with up to `MIRROR_MAX_BOUNCES 4` extra legs, each with fresh `LASER_REACH` ("mirrors give the beam new legs"); every leg applies full `beamSegmentEffects` (pop/ignite/cut/light-cannon). Perfectly edge-on hits skim past. Solid body; otherwise inert |
