...
├── test/
│   ├── smoke.mjs            101 physics behaviour proofs (run in Node)
...
| `switch` | 84×20 | ✓ | ✓ (sandbox) | – | – | pressure plate; wires to nearest fan/conveyor/magnet/hydrant/laser/bulb within 260px at sim start; device runs only while pressed (magnet: switch replaces bump/timer). MEMORY FOAM: anything LANDING on the plate (falling, center above it) has its fall killed and roll/spin damped ×0.4 in collisionStart — Matter pairs resolve with MAX restitution, so bouncy balls used to chatter or ricochet clean off; plus a per-frame brake (vx and spin ×0.82) while anything touches the plate from above — ROLLING bodies are caught and stopped at the plate instead of crossing it — and a 5-frame release debounce (`sw.linger`) so an absorbed landing´s ~1px micro-hop never blips the plate |
...
- **Machine shop (sandbox parts).** All wired in `createSim` (ropes tether the
  nearest dynamic body; switches link the nearest fan/conveyor/magnet/hydrant/laser/bulb,
...
```bash
node test/smoke.mjs      # 101 physics behaviour proofs — every part & interaction
